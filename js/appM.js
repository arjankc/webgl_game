...
var obstacles = [];
	...
	var 	animOffset      = 0,   // starting frame of animation
	        duration        = 1000, // milliseconds to complete animation
	  		...
	  		currentKeyframe = 0,
	  		bird,bird_material, bird2, bird_material2;
	...
	function initScene(){
			...
			jsonLoader2.load( "models/parrot.js", function(geometry, materials){
			  ...
			  geometry.computeMorphNormals();
			  addModelToScene2(geometry, materials);
			...
			var materialFront = new THREE.MeshBasicMaterial( { color: 0xff00ff } );
				var materialSide = new THREE.MeshBasicMaterial( { color: 0x0ff088 } );
				var materialArray = [ materialFront, materialSide ];
				var textGeom = new THREE.TextGeometry( "Start NOW", 
				{
	...
	function theBase(){
		var z_value_floor = 0;
		for(var i=0; i<=150;i++){
			var floorBase = new THREE.Mesh(new THREE.CubeGeometry( 275, 50, 5), 
	...
	function addModelToScene2( geometry, materials ){
		...
		bird_material2  = new THREE.MeshPhongMaterial
		    ({ 
		      color: 0xffffff, morphTargets: true, morphNormals: true, 
		      vertexColors: THREE.FaceColors, shading: THREE.SmoothShading 
		    });

		bird2 = new THREE.Mesh( geometry,bird_material );
		bird2.castShadow  = true;

		scene.add(bird2);

		bird2.scale.set(0.5,0.5,0.5);
		bird2.rotation.set(0,6.2,0);
		bird2.position.set(-40,0,0);
	}
	...
	function render(t) {
		...
		if(bird2){
			...
			var keyframe = Math.floor( time / interpolation ) + animOffset;
			if ( keyframe != currentKeyframe ){
			  bird2.morphTargetInfluences[ lastKeyframe ] = 0;
			  bird2.morphTargetInfluences[ currentKeyframe ] = 1;
			  bird2.morphTargetInfluences[ keyframe ] = 0;
			  lastKeyframe = currentKeyframe;
			  currentKeyframe = keyframe;
			}
			bird2.morphTargetInfluences[ keyframe ] = ( time % interpolation ) / interpolation;
			bird2.morphTargetInfluences[ lastKeyframe ] = 1 - bird2.morphTargetInfluences[ keyframe ];
